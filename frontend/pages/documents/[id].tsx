...
export default function DocumentDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<"prediction" | "analysis" | "confidence">("prediction");
    ...
    return (
      <AppLayout title="Loading...">
        <div style={{ textAlign: "center", padding: "80px 0" }}>
          <span className="spinner" style={{ width: 40, height: 40, margin: "0 auto", display: "block" }} />
        </div>
      </AppLayout>
    );
  }
    ...
    <>
      ...
      <AppLayout title={doc?.original_filename || "Document Detail"}>
        ...
        >
          ...
          <MiniStat
            label="Pages"
            value={analysisResult?.num_pages_processed?.toString() || "—"}
          />
          <MiniStat
            label="Papers Found"
            value={analysisResult?.num_papers_found?.toString() || "—"}
          />
          <MiniStat
            label="Confidence"
            value={
        ...
        >
          {(["prediction", "analysis", "confidence"] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === "prediction" ? "📝 Predicted Paper" : tab === "analysis" ? "🔬 Analysis" : "📊 Confidence"}
            </button>
          ))}
        </div>
          ...
          <div className="animate-fade-in">
            ...
            >
              <h2 style={{ fontSize: "1.2rem", fontWeight: 800, margin: 0 }}>
                {paper.paper_info?.title || "Predicted Question Paper"}
              </h2>
              <p style={{ color: "var(--accent-violet)", fontSize: "1rem", margin: "4px 0 12px" }}>
              ...
              >
                <span>📅 {paper.paper_info?.academic_year || ""}</span>
                <span>⏱ {paper.paper_info?.duration || ""}</span>
                <span>📊 Max Marks: {paper.paper_info?.max_marks || ""}</span>
              </div>
            </div>

            {/* Sections */}
            {paper.sections?.map((section: any, sIdx: number) => (
              <div key={sIdx} className="glass-card" style={{ padding: 24, marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  ...
                  <span className="badge badge-info">{section.total_marks} marks</span>
                </div>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          ...
                          {q.confidence !== undefined && (
                            <p style={{ margin: "4px 0 0", fontSize: "0.7rem", color: getConfidenceColor(q.confidence) }}>
        ...
        {activeTab === "analysis" && (
          <div className="animate-fade-in">
            ...
            {topicData?.chart_data?.labels?.length > 0 && (
              <div className="glass-card" style={{ padding: 24, marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {topicData.chart_data.labels.map((label: string, i: number) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div style={{ flex: 1, position: "relative" }}>
                        <div
                          ...
                          style={{
                            width: `${Math.max(
                              (topicData.chart_data.values[i] /
                                Math.max(...topicData.chart_data.values)) *
                                100,
                              4
                            )}%`,
                            background: topicData.chart_data.colors[i],
                          }}
                        />
                      </div>
                      <span
                      ...
                      >
                        {topicData.chart_data.values[i]}x ({topicData.chart_data.percentages[i]}%)
                      </span>
                      <TrendBadge trend={topicData.chart_data.trends[i]} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="glass-card" style={{ padding: 24 }}>
              ...
              >
                <MetaItem label="Model Used" value={analysisResult?.model_used || "—"} />
                <MetaItem
                ...
                <MetaItem
                  ...
                  value={
                    analysisResult?.completed_at
                      ? new Date(analysisResult.completed_at).toLocaleString()
                      : "—"
                  }
                />
              </div>
        ...
        {activeTab === "confidence" && confidence && (
          <div className="animate-fade-in">
            ...
            <div className="glass-card" style={{ padding: 32, textAlign: "center", marginBottom: 20 }}>
              <ConfidenceRing value={confidence.overall_confidence || prediction?.overall_confidence || 0} />
            ...
            <div className="glass-card" style={{ padding: 24, marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <ConfidenceBar
                  ...
                  value={confidence.historical_alignment_score || 0}
                  description="Match with past exam patterns"
                />
                <ConfidenceBar
                  label="Question Quality"
                  value={confidence.question_quality_score || 0}
            ...
            {confidence.per_question_confidence?.length > 0 && (
              <div className="glass-card" style={{ padding: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {confidence.per_question_confidence.map((q: any, i: number) => (
                    <div
                      key={i}
                      style={{
                        display: "flex",
        ...
        {!prediction && !loading && (
          <div className="glass-card" style={{ padding: "60px 24px", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "1.1rem", fontWeight: 600 }}>No prediction generated yet</p>
            <p style={{ color: "var(--text-muted)", marginBottom: 20 }}>
              Run the analysis pipeline to generate a predicted paper
            </p>
          </div>
        )}
      </AppLayout>
...
function MiniStat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="stat-card">
      ...
      <div className={`stat-value ${highlight ? "text-gradient" : ""}`} style={{ fontSize: "1.5rem" }}>
        {value}
      </div>
    </div>
  );
...
function MetaItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p style={{ color: "var(--text-muted)", fontSize: "0.75rem", margin: "0 0 2px" }}>{label}</p>
      <p style={{ color: "var(--text-primary)", fontSize: "0.85rem", margin: 0, fontWeight: 500 }}>{value}</p>
    </div>
  );
}
...
function TrendBadge({ trend }: { trend: string }) {
  ...
  return (
    <span style={{ color: c.color, fontSize: "0.8rem", fontWeight: 700, width: 20, textAlign: "center" }}>
      {c.label}
    </span>
  );
}
...
function ConfidenceRing({ value }: { value: number }) {
  ...
  return (
    <div style={{ display: "inline-block", position: "relative" }}>
      ...
      >
        <span style={{ fontSize: "1.75rem", fontWeight: 800, color: getConfidenceColor(value) }}>{pct}%</span>
      </div>
    </div>
  );
}

function ConfidenceBar({ label, value, description }: { label: string; value: number; description: string }) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        <span style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--text-primary)" }}>{label}</span>
        <span style={{ fontSize: "0.85rem", fontWeight: 700, color: getConfidenceColor(value) }}>
          {(value * 100).toFixed(0)}%
        </span>
      </div>
      ...
      <p style={{ color: "var(--text-muted)", fontSize: "0.7rem", marginTop: 4 }}>{description}</p>
    </div>
  );
}
