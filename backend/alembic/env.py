...
Runs migrations against the async (asyncpg) engine so no separate sync driver
(psycopg2) is required. Offline mode renders SQL from the URL string alone.
"""
...
# Use the async (asyncpg) URL — env.py drives an async engine below.
